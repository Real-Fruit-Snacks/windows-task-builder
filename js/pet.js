(function () {
  ...
  function readSize() {
    var s = parseInt(localStorage.getItem("task-pet-size"), 10);
  ...
  var COLOR_COUNT = 6;
  var petColor = 0;
  try { petColor = parseInt(localStorage.getItem("task-pet-color"), 10) || 0; }
  ...
  function cyclePetColor() {
    ...
    applyPetColor();
    try {
      if (petColor) localStorage.setItem("task-pet-color", String(petColor));
      else localStorage.removeItem("task-pet-color");
    } catch (e) { /* private mode */ }
  }
  ...
  function applyOpacity() {
    var o = parseInt(localStorage.getItem("task-pet-opacity"), 10);
    if (!(o >= 15 && o <= 100)) o = 70;
    document.documentElement.style.setProperty("--pet-base-opacity", (o / 100).toFixed(3));
  }
  function readCfg() {
    cfgNap = boolKey("task-pet-nap", true);
    cfgFlee = boolKey("task-pet-flee", true);
    cfgRead = boolKey("task-pet-read", true);
    cfgTricks = boolKey("task-pet-tricks", true);
    cfgSpeech = boolKey("task-pet-speech", false);
    var c = parseInt(localStorage.getItem("task-pet-color"), 10);
